...
for(s in 1:length(unique(SpDayReg$species))){
           ...
           stan_glm(
               formula = round(har) ~
                   bag.limit + min.size + pr.wave + pr.year +
                       bag.limit*min.size + bag.limit+pr.wave + bag.limit*pr.year +
                       min.size*pr.wave + min.size*pr.year+
                       pr.wave*pr.year +
    ...
    pdf(file = paste0('postcleanrevision/', unique(SpDayReg$species)[s], '.pdf'))
...
summary(observation)
save.image(file = 'outrevision/dayspecies.RData')
